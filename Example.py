...
def app():
    ...
    st.markdown("""## Here is a demo of how this app works.
                   \n The company's internal dataset looks like the following. I catogorize the features into three groups:
                   \n * **Bid and Rank**: Thease two are the most important features which will be used to train the model.
                   \n * **End user charactersitics**: These will be used to create customer groups in order to
                   train the model for each customer group and make customer-specific analysis. 
                   My model can handle both numerical and categorical features.
                   \n * **No-use**: Features with no useful information, such as `impression_id`, duplicated columns, etc.""")
    
    df = pd.read_excel('Bidding.xls')
    df['cost_duplicated'] = df['cost']
    #df['cost_made_up'] = np.random.randint(5,20,len(df))
    
    st.dataframe(df.head())
    
    #st.write('Columns List: \n', list(df.columns))
    
    #bidcolname = st.selectbox("Choose bid amount column", list(df.columns), 9) #2, 9
    #rankcolname = st.selectbox("Choose rank column", list(df.columns), 7)
    
#    with open('SimulationEstimator.pkl' ,'rb') as f:
#        simest = dill.load(f)
    
    #fig, ax = plt.subplots()
    #ax.hist(simest.simulated_bids, bins = 20)
    #st.pyplot(fig)
    
    st.markdown("""## Specify the feature categories.
                   \n First specify bid and rank columns.""")

    col1, col2 = st.beta_columns(2)
    with col1:
        bidcolname = st.selectbox("Choose bid amount column", list(df.columns),2) #2, 9
    with col2:
        rankcolname = st.selectbox("Choose rank column", list(df.columns),7)
        
    st.markdown(""" Then specify the columns that are not used in the training. 
                    These include the columns that are irrelevant, 
                    as well as those which are not related to user types but can be used in analysis.
                    After selecting these features, only the features related to customer-types will be left for analysis.""")
    
    features_to_ignore = st.multiselect(
        r"Choose the irrelevant features; e.g. `id`",
        [item for item in list(df.columns) if item not in [bidcolname, rankcolname]],
        default=['impression_id', 'cost_duplicated'])
    features_to_pass_on = st.multiselect(
        "Choose the relevant features that won't be used in model training; e.g. `click`, `purchased`, etc.", 
        [item for item in list(df.columns) if item not in features_to_ignore + [bidcolname, rankcolname]], 
        default=['click', 'policies sold'])
    
    st.write('Based on your choices, the following features are related to customer characteristics. Correct? ',
             [item for item in list(df.columns) if item not in features_to_pass_on + features_to_ignore + [bidcolname, rankcolname]])
    ...
    st.markdown(""" ## Model Results:
                    ...
                    To inspect how good the model is fit, we can check the rank frequency observed in the data 
                    against that produced by the model. Below we observe that for all the user groups combined.
                    """)
...
#        dill.dump(simest, open(filename, 'wb'))
    ...
    with st.beta_expander("See model fit for each groups separately : "):
        ...
        ax = ax.ravel()
        j = 0
        for group in cgm.customer_groups:
            ...
            ax[j].bar([i[0] for i in ranks_count_data], [i[1] for i in ranks_count_data], alpha=0.5,color='b')
            ax[j].bar([i[0] for i in ranks_count_simu], [i[1] for i in ranks_count_simu], alpha=0.5,color='r')
            ax[j].set_title(cgm.customer_groups[j])
            j += 1
